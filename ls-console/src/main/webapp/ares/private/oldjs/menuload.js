...
function initMenu(menus_var) {
	...
	$.each(menus_var, function(i, n) {// 外层循环处理一级菜单
		var menulist = '';
		menulist += '<ul>';
		$.each(n.menuList, function(j, o) {// 二层处理二级菜单
			// 这里自定义了一些属性存放菜单的内容：title存放菜单名称rel存放菜单地址，这些在属性在点击菜单 时要取出值使用
			menulist += '<li><div>' 
				+ '<a title="' + o.menuname + '" ref="' + o.menuid + '" href="#" rel="' + o.url + '" icon="' + o.icon + '"  >' 
				+ '<span class="icon ' + o.icon + '" >&nbsp;</span>'
				+ '<span class="nav">' + o.menuname	+ '</span></a>'
				+ '</div></li> ';
		});
