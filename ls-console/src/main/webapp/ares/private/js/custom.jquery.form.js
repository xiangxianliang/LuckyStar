...
function loading() {
	$("body").append(
			'<div  id="load" style="width:100%;position:absolute;top:250px;text-align:center;">'
					+ '<img src="/ares/png/images/loading2.gif" /></div>');
	// '<img src="' + BaseUrl + 'png/images/loading2.gif" /></div>');
}
 ...
 * form提交(post方式)
 * 
 * formname form Name callbackfn
 * 回调函数名(要求函数必须有参数且不能多与两个,一个参数时参数为响应文本,两个参数时第一个参数为响应文本) param
 * 回调函数参数(如果为null,那么调用一个参数的回调函数,否则调用两个参数的回调函数)
 */
