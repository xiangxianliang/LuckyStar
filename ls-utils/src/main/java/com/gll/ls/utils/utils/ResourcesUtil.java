...
public class ResourcesUtil implements Serializable {
	...
	static {
		if (system == null) {
			...
			ResourceBundle rb = ResourceBundle.getBundle(Config.AUTHORITY_CONSOLE_SYSTEM, locale);
			Set<String> keySet = rb.keySet();
			for (String key : keySet) {
				String retValue = (String) rb.getObject(key);
				system.put(key, retValue);
			}
		}
		if (version == null) {
			...
			// 初始化 systemMap()
			Locale locale = getLocale();
			ResourceBundle rb = ResourceBundle.getBundle(Config.BACK_VERSION, locale);
			Set<String> keySet = rb.keySet();
			for (String key : keySet) {
				String retValue = (String) rb.getObject(key);
				system.put(key, retValue);
